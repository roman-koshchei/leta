...
const Create: NextPage = () => {
  ...
  const analyzeLayout = async () =>
    setAnalysis(await analyze(keysToLayout(name)))
        ...
        <div className="mt-10 w-full">
          {selected.col != -1 ? (
            <div className="flex justify-between items-center mb-10">
              Hand:{" "}
              {keys[selected.row][selected.col].finger > 3 ? "Right" : "Left"}
              <div className="flex gap-2">
                {Object.values(Finger).map((f) =>
                  typeof f != "string" ? (
                    <FingerButton key={f} finger={f} />
                  ) : (
                    <></>
                  )
                )}
              </div>
              <TextButton onClick={() => setSelected({ row: -1, col: -1 })}>
                Close
              </TextButton>
            </div>
          ) : (
